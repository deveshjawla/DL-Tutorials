...
# Just in case not already loaded
@load LogisticClassifier pkg=MLJLinearModels

# Assuming: 
# - X2 is the feature DataFrame (excluding Year and Today)
...
cv = CV(nfolds=10, shuffle=true, rng=123)

evaluation = evaluate!(mach,
    resampling=cv,
    measures=[accuracy, misclassification_rate, cross_entropy],
    operations=[predict_mode, predict_mode, predict],
    verbosity=1)
